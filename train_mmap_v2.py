...
class TrainMain:
    ...
    def define_model_optimizer_scheduler(self):
        ...
        if torch.cuda.device_count() > 1:
            ...
            self.model["ST"] = DDP(self.model["ST"],device_ids=[self.local_rank],output_device=self.local_rank) 
    ...
    def _build_dataset(self, subset: str)-> STDataset:
    ...
    def _set_data_loader(self, dataset, collater):
        self.data_loader = {
            ...
            "dev": DataLoader(
                ...
                collate_fn=collater,
                batch_size=self.config["batch_size"],
                sampler = DistributedSampler(dataset["dev"])
            ),
        }
